...
export default function AddLangualCode({
...
}: NewLangualCodeProps): JSX.Element {
    ...
    const getChildCodes = (parentId: number): LangualCode[] =>
        langualCodes.filter(code => code.parentId === parentId);

    const getFilteredParentAndChildren = (): LangualCode[] => {
        if (!searchTerm) return parentCodes;

        return parentCodes.filter(parent => {
            const parentMatches = matchesSearchTerm(parent.code);
    ...
    const getFilteredChildCodes = (parentId: number): LangualCode[] => {
    ...
    const toggleExpand = (parentId: number): void => {
    ...
    const handleChildSelection = (childId: number): void => {
        onLangualCodesChange(childId);
    };

    const handleChildPageChange = (parentId: number, page: number): void => {
        setChildrenPages(prev => ({ ...prev, [parentId]: page }));
    };

    const ITEMS_PER_PAGE = 5;

    const getPaginatedChildren = (parentId: number): PaginatedChildren => {
