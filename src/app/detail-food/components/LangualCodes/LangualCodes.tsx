                        ...
                        const childRows = lcode.children.length > 1
                            ? lcode.children.slice(1).map((lchild, index) => (
                                ...
                                >
                                    ...
                                    <td className={`${styles.childCell} ${styles.childCellIndented} ${styles.classificationColumn}`}>
                                            <span>
                                                <span className={styles.childCode}>{lchild.code}</span>, {lchild.descriptor}
                                            </span>
                                    </td>
                                </tr>
                            ))
                                ...
                                <div className={styles.cardRow}>
                                    ...
                                    <div className={styles.cardValue}>
                                        {lcode.children.map((child, index) => (
                                            <div key={`child-card-${child.code}-${index}`} className={index > 0 ? styles.childCard : ''}>
                                                <span className={styles.childCode}>{child.code}</span>, {child.descriptor}
                                            </div>
                                        ))}
                                    </div>
                                </div>
